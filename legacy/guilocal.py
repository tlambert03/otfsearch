...
def send_command(remotefile, mode):
	...
	if mode == 'registerCal':
		...
		if RegFile.get().strip(): 
			command.extend(['--regfile', RegFile.get(), '-r', RefChannel.get()])
	...
	statusTxt.set("Starting command...")
	...
	#root.after(500, receive_command_response)
...
def getRegFile():
	...
	reglist = sorted([item for item in os.listdir(C.regFileDir) if item.endswith('.mat')])
...
def getChannelOTF(var):
	...
	otflist = sorted([item for item in os.listdir(OTFdir.get()) if item.endswith('.otf')])
...
def get_recent_regfile():
	reglist = sorted([item for item in os.listdir(C.regFileDir) if item.endswith('.mat') and "refs" not in item])
